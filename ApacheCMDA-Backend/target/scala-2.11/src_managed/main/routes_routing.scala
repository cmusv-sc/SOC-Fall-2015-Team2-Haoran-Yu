...
// @LINE:65
private[this] lazy val controllers_CommentController_getAllComments42_route = Route("GET", PathPattern(List(StaticPart(Routes.prefix),StaticPart(Routes.defaultPrefix),StaticPart("users/getAllComment/json"))))
private[this] lazy val controllers_CommentController_getAllComments42_invoker = createInvoker(
...
// @LINE:65
case controllers_CommentController_getAllComments42_route(params) => {
   call(Param[String]("format", Right("json"))) { (format) =>
        controllers_CommentController_getAllComments42_invoker.call(play.api.Play.maybeApplication.map(_.global).getOrElse(play.api.DefaultGlobal).getControllerInstance(classOf[controllers.CommentController]).getAllComments(format))
   }
}
